...
exports.fetchPlatformAnalytics = fetchPlatformAnalytics;
async function apiError(res, label) {
    const body = await res.text();
    throw new Error(`${label} API error (${res.status}): ${body.slice(0, 300)}`);
}
async function publishToPlatform(platform, content, token) {
    switch (platform) {
        case 'X':
...
async function fetchPlatformAnalytics(platform, token) {
    switch (platform) {
        case 'X':
...
async function publishX(content, token) {
...
async function publishLinkedIn(content, token) {
    const meRes = await fetch('https://api.linkedin.com/v2/userinfo', {
        headers: { authorization: `Bearer ${token}` },
    });
    if (!meRes.ok)
        await apiError(meRes, 'LinkedIn profile');
    ...
    if (!res.ok)
        await apiError(res, 'LinkedIn');
    const postId = res.headers.get('x-restli-id') ?? crypto.randomUUID();
    return { platform: 'LinkedIn', postId };
}
async function publishFacebook(content, token) {
    const pagesRes = await fetch(`https://graph.facebook.com/v19.0/me/accounts?access_token=${encodeURIComponent(token)}`);
    if (!pagesRes.ok)
        await apiError(pagesRes, 'Facebook pages');
...
async function publishInstagram(content, token) {
    const pagesRes = await fetch(`https://graph.facebook.com/v19.0/me/accounts?fields=id,instagram_business_account&access_token=${encodeURIComponent(token)}`);
    if (!pagesRes.ok)
        await apiError(pagesRes, 'Instagram account lookup');
    const pages = (await pagesRes.json());
    ...
    throw new Error('Instagram publishing requires a media container (image/video). Use draft + manual media upload for now.');
}
async function publishYouTube(content, token) {
    throw new Error('YouTube publishing requires video upload via Data API. Post metadata-only updates are not supported.');
}
async function publishTikTok(content, token) {
...
async function analyticsLinkedIn(token) {
    return {
